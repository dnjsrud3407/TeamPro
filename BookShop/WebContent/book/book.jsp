...
<!-- fav and touch icons -->
	...
	function openQna(p, number) {
		...
		if(loginChk()){
 			window.open('',"bookQnaForm",'toolbar=no,scrollbars=no,resizable=no,top=100,left=0,width=750,height=600');
				...
				<form class="form-horizontal qtyFrm" method="post" name="kindFrm">
				  <div class="control-group">
					...
					<input type="hidden" name="bookID" value="${book.bookID }"/>
					  ...
					  <button type="button" class="btn btn-large btn-primary pull-right" onclick='kindSubmit(2,qty.value)'>Add to <i class=" icon-shopping-cart"></i></button>
					 ...
					 <div class="tab-pane fade" id="bookqna">
								...
								<td colspan="4"> 		Q ${qna.boardContent}<br>
								 <c:if test="${qna.boardReSeq == 1}">
									...
									<c:if test="${sessionScope.uID eq qna.boardWriter}">
							    	 <button style="width: 100px;" class="btn dropdown-toggle" data-toggle="dropdown" onclick="openQna('m','<c:out value="${qna.boardNum}"/>')" >글 수정하기 <span class=""></span></button>
							         <button style="width: 100px;" class="btn dropdown-toggle" data-toggle="dropdown" onclick="openQna('d','<c:out value="${qna.boardNum}"/>')">글 삭제하기 <span class=""></span></button>
							    	</c:if>
									</td>
								</tr>
					   ...
					   <%--  <a href="BookList.book?bk2=${book.BK2 }" class="btn btn-large pull-right">Compair Product</a> --%>
						...
						<div class="pagination txCenter">
							<ul>
							 <c:if test="${pageInfoQna.startPage > pageBlock }">
								<li>
								  <a href='<c:url value="/Book.book?bookID=${book.bookID }&page=${pageInfoQna.startPage-pageInfoQna.pageBlock }"/>'>&lsaquo;</a>
								  </li>
							 </c:if>
							<li>
							...
							<a href='<c:url value="/Book.book?bookID=${book.bookID}&page=${i}"/>'>${i}</a>
								...
								<c:if test="${pageInfoQna.endPage < pageInfoQna.maxPage }">
								  <li>
									<a href='<c:url value="/Book.book?bookID=${book.bookID }&page=${pageInfoQna.startPage + pageInfoQna.pageBlock}"/>'>&rsaquo;</a>
								  </li>
								</c:if>
							</ul>
							</div>
