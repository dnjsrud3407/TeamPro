        ...
        <div class="container-fluid">
          ...
          <h1 class="h3 mb-2 text-gray-800">FAQ</h1>
          	...
          	<table class="table table-bordered" id="dataTable" width="50%" cellspacing="0">		
              	<tbody>
              	<tr>
              		<td><a href='<c:url value="/FAQList.bo"/>'>전체</a></td>
	              	<c:forEach var="k2" items="${k2List }" varStatus="k2Status">
	              	
		              	<td>
		              		<a href='<c:url value="/FAQList.bo?k2=${k2}"/>'>${k2}</a>
		              	</td>
					</c:forEach>
                ...
                <table class="table table-bordered" id="dataSearchTable" width="100%" cellspacing="0">
              	...
              	<div class="card-header py-3">
                	...
                	<c:if test="${pageInfo.startPage > pageInfo.pageBlock }">
                		<a href="FAQList.bo?page=${pageInfo.startPage-pageInfo.pageBlock }">[이전]</a>&nbsp;
                	</c:if>
                	<c:forEach var="i" begin="${pageInfo.startPage }" end="${pageInfo.endPage }" step="1">
                	<c:choose>
                	<c:when test="${pageInfo.page eq i }">${i }&nbsp;</c:when>
                	<c:when test="${pageInfo.k2 eq null }"> <a href="FAQList.bo?page=${i }">${i }</a>&nbsp; </c:when>
                	<c:when test="${pageInfo.k2 != null }"> <a href="FAQList.bo?page=${i }&k2=${pageInfo.k2}">${i }</a>&nbsp; </c:when>
                	<c:otherwise><a href="FAQList.bo?page=${i }">${i }</a>&nbsp;</c:otherwise>
                	</c:choose>
                		
                	</c:forEach>
                	<c:if test="${pageInfo.endPage < pageInfo.maxPage }">
                		<a href="FAQList.bo?page=${pageInfo.startPage+pageInfo.pageBlock }">[다음]</a>
                	</c:if>
                	</section>
                	<div style="text-align:right">
