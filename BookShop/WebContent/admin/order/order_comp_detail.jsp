        ...
        <div class="container-fluid">
          ...
          <div class="card shadow mb-4">
           <div class="card-header py-3">
				<h6 class="m-0 font-weight-bold text-primary">-거래 상세보기-</h6>
			</div>
            <div class="card-body">
              <div class="table-responsive">
              <form action="OrderCompDetailModifyPro.adm" method="post" id="searchForm">
...
<%-- 		    			    <option value="null" class="size" <c:if test="${orderDetaile.orderStatus eq '선택해주세요' }">selected</c:if>>선택해주세요.</option>				 --%>
		    			   ...
		    			   if(orderDetaile.getOrderStatus().equals("결제완료") || orderDetaile.getOrderStatus().equals("배송중") || orderDetaile.getOrderStatus().equals("배송완료")){
							...
							<option value="배송완료" <c:if test="${orderDetaile.orderStatus eq '배송완료' }">selected</c:if>>배송완료</option>		
		    				   ...
		    				   <%
		    			   } else if(orderDetaile.getOrderStatus().equals("취소요청") || orderDetaile.getOrderStatus().equals("취소완료")){
 								%>
							...
							<option value="취소완료" <c:if test="${orderDetaile.orderStatus eq '취소완료' }">selected</c:if>>취소완료</option>
		    				   ...
		    				   <%
		    			   } else if(orderDetaile.getOrderStatus().equals("반품요청") || orderDetaile.getOrderStatus().equals("반품완료")){
 								%>
							...
							<option value="반품완료" <c:if test="${orderDetaile.orderStatus eq '반품완료' }">selected</c:if>>반품완료</option>
		    				   ...
		    				   <%
		    			   } else if(orderDetaile.getOrderStatus().equals("교환요청") || orderDetaile.getOrderStatus().equals("교환완료")){
 								%>
...
// 		    			   else if(orderDetaile.getOrderStatus().equals("주문완료")){
								%>
...
<%-- 							<option value="결제완료" <c:if test="${orderDetaile.orderStatus eq '주문완료' }">selected</c:if>>결제완료 </option> --%>
							
<%-- 							<option value="배송중" <c:if test="${orderDetaile.orderStatus eq '배송중' }">selected</c:if>>배송중</option>		 --%>
<%-- 							<option value="배송완료" <c:if test="${orderDetaile.orderStatus eq '배송완료' }">selected</c:if>>배송완료</option>		 --%>
							
<%-- 							<option value="취소중" <c:if test="${orderDetaile.orderStatus eq '취소진행중' }">selected</c:if>>취소요청</option> --%>
<%-- 							<option value="취소중" <c:if test="${orderDetaile.orderStatus eq '취소완료' }">selected</c:if>>취소요청</option> --%>
													
<%-- 							<option value="배송내역" <c:if test="${orderDetaile.orderStatus eq '반품진행중' }">selected</c:if>>배송내역</option> --%>
<%-- 							<option value="반품요청" <c:if test="${orderDetaile.orderStatus eq '반품완료' }">selected</c:if>>반품요청</option> --%>
						
<%-- 							<option value="교환요청" <c:if test="${orderDetaile.orderStatus eq '교환진행중' }">selected</c:if>>교환요청</option> --%>
<%-- 							<option value="반품요청" <c:if test="${orderDetaile.orderStatus eq '교환완료' }">selected</c:if>>반품요청</option> --%>
	                  ...
	                  <tr>
	                   ...
	                   <tr>
	                    ...
	                    <tr>
	                  	...
	                  	<td colspan="3">
	                  		<span class="checkbox_padding" style="width: 15%;">
                  				<%=memberInfo.getAddress_post() + memberInfo.getAddress_1() + memberInfo.getAddress_2() %>
                  			</span>
	                  	</td>
	                  </tr>
                  </table>
